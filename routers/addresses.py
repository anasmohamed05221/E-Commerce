...
async def create_address(request: Request, db: db_dependency, tenant: tenant_dependency, current_user: customer_dependency, data: AddressCreate):
    """Create a new address for the current user."""
    address = await AddressService.create_address(db=db, tenant_id=tenant.id, user_id=current_user.id, data=data)
...
async def get_addresses(request: Request, db: db_dependency, tenant: tenant_dependency, current_user: customer_dependency):
...
@limiter.limit("30/minute")
async def get_address(request: Request, db: db_dependency, tenant: tenant_dependency, current_user: customer_dependency, address_id: int):
...
@limiter.limit("10/minute")
async def update_address(request: Request, db: db_dependency, tenant: tenant_dependency, current_user: customer_dependency, address_id: int, data: AddressUpdate):
    """Partially update an address. Returns 404 if not found or not owned by the user."""
    address = await AddressService.update_address(db=db, tenant_id=tenant.id, user_id=current_user.id, address_id=address_id, data=data)
...
@limiter.limit("10/minute")
async def delete_address(request: Request, db: db_dependency, tenant: tenant_dependency, current_user: customer_dependency, address_id: int):
...
@limiter.limit("10/minute")
async def set_default_address(request: Request, db: db_dependency, tenant: tenant_dependency, current_user: customer_dependency, address_id: int):
    """Set an address as the current user's default. Returns 404 if not found or not owned by the user."""
    address = await AddressService.set_default(db=db, tenant_id=tenant.id, user_id=current_user.id, address_id=address_id)
