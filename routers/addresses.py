...
def create_address(request: Request, db: db_dependency, current_user: customer_dependency, data: AddressCreate):
    """Create a new address for the current user."""
    address = AddressService.create_address(db=db, user_id=current_user.id, data=data)
...
def get_addresses(request: Request, db: db_dependency, current_user: customer_dependency):
    """Return all addresses belonging to the current user."""
    return AddressService.get_addresses(db=db, user_id=current_user.id)


@router.get("/{address_id}", response_model=AddressOut, status_code=status.HTTP_200_OK)
@limiter.limit("30/minute")
def get_address(request: Request, db: db_dependency, current_user: customer_dependency, address_id: int):
...
def update_address(request: Request, db: db_dependency, current_user: customer_dependency, address_id: int, data: AddressUpdate):
    ...
    address = AddressService.update_address(db=db, user_id=current_user.id, address_id=address_id, data=data)
...
@limiter.limit("10/minute")
def delete_address(request: Request, db: db_dependency, current_user: customer_dependency, address_id: int):
    """Delete an address. Returns 404 if not found or not owned by the user."""
    AddressService.delete_address(db=db, user_id=current_user.id, address_id=address_id)
...
@limiter.limit("10/minute")
def set_default_address(request: Request, db: db_dependency, current_user: customer_dependency, address_id: int):
    """Set an address as the current user's default. Returns 404 if not found or not owned by the user."""
    address = AddressService.set_default(db=db, user_id=current_user.id, address_id=address_id)
