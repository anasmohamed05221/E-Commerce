...
async def create_product(request: Request, db: db_dependency, tenant: tenant_dependency, admin: admin_dependency, body: ProductCreate):
    """Create a new product. Admin only."""
    product = await ProductService.create_product(db, body, tenant.id)
...
@limiter.limit("30/minute")
async def update_product(request: Request, db: db_dependency, tenant: tenant_dependency, admin: admin_dependency, body: ProductUpdate, product_id: int):
    """Update an existing product. Admin only."""
    product = await ProductService.update_product(db, body, product_id)
...
@limiter.limit("30/minute")
async def delete_product(request: Request, db: db_dependency, tenant: tenant_dependency, admin: admin_dependency, product_id: int):
