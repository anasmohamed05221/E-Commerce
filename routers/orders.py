...
async def checkout(request: Request, db: db_dependency, current_user: customer_dependency, body: CheckoutRequest):
    """Place an order from the current user's active cart."""
    order = await CheckoutService.checkout(db, current_user.id, body.address_id, body.payment_method)
...
@limiter.limit("60/minute")
async def get_order(request: Request, db: db_dependency, current_user: customer_dependency, order_id: int):
...
async def cancel_order(request: Request, db: db_dependency, current_user: customer_dependency, order_id: int):
