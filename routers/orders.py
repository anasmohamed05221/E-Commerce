...
async def checkout(request: Request, db: db_dependency, tenant: tenant_dependency, current_user: customer_dependency, body: CheckoutRequest):
    """Place an order from the current user's active cart."""
    order = await CheckoutService.checkout(db, tenant.id, current_user.id, body.address_id, body.payment_method)
...
async def get_orders(request: Request,
    ...
    orders, total = await OrderService.get_orders(db=db, user_id=current_user.id, limit=limit, offset=offset)
...
@limiter.limit("60/minute")
async def get_order(request: Request, db: db_dependency, tenant: tenant_dependency, current_user: customer_dependency, order_id: int):
    """Return full details for a single order, including all items. Returns 404 if not found or not owned by the user."""
    order = await OrderService.get_order(db=db, user_id=current_user.id, order_id=order_id)
...
async def cancel_order(request: Request, db: db_dependency, tenant: tenant_dependency, current_user: customer_dependency, order_id: int):
    """Cancel a pending order and restore stock for all items. Returns 409 if the order is not cancellable."""
    cancelled_order = await OrderService.cancel_order(db=db, user_id=current_user.id, order_id=order_id)
