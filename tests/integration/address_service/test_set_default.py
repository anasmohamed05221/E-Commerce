...
def test_set_default_success(session, verified_user, test_address):
    """Returns the address with is_default=True."""
    result = AddressService.set_default(session, verified_user.id, test_address.id)

    assert result.is_default is True
    assert result.id == test_address.id


def test_set_default_clears_previous_default(session, verified_user, test_address):
    ...
    AddressService.set_default(session, verified_user.id, second.id)
...
def test_set_default_not_found(session, verified_user):
    """Raises 404 for a non-existent address ID."""
    with pytest.raises(HTTPException) as exc:
        AddressService.set_default(session, verified_user.id, 99999)

    assert exc.value.status_code == 404


def test_set_default_wrong_owner(session, verified_user):
    ...
    with pytest.raises(HTTPException) as exc:
        AddressService.set_default(session, verified_user.id, other_address.id)
