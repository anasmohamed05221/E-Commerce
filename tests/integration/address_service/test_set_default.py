...
async def test_set_default_success(session, verified_user, test_address, test_tenant):
    """Returns the address with is_default=True."""
    result = await AddressService.set_default(session, tenant_id=test_tenant.id, user_id=verified_user.id, address_id=test_address.id)

    assert result.is_default is True
    assert result.id == test_address.id


async def test_set_default_clears_previous_default(session, verified_user, test_address, test_tenant):
    ...
    await AddressService.set_default(session, tenant_id=test_tenant.id, user_id=verified_user.id, address_id=second.id)
...
async def test_set_default_not_found(session, verified_user, test_tenant):
    """Raises 404 for a non-existent address ID."""
    with pytest.raises(HTTPException) as exc:
        await AddressService.set_default(session, tenant_id=test_tenant.id, user_id=verified_user.id, address_id=99999)

    assert exc.value.status_code == 404


async def test_set_default_wrong_owner(session, verified_user, test_tenant):
    ...
    with pytest.raises(HTTPException) as exc:
        await AddressService.set_default(session, tenant_id=test_tenant.id, user_id=verified_user.id, address_id=other_address.id)
