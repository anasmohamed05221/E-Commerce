...
async def test_get_cart_returns_items_with_product_loaded(session, verified_user, product_factory, test_tenant):
    ...
    assert cart_items[0].product.name == product.name
...
async def test_calculate_total_price_multiple_items(session, verified_user, product_factory, test_tenant):
    ...
    await CartService.add_to_cart(db=session, tenant_id=test_tenant.id, user_id=verified_user.id, product_id=p1.id, quantity=2)
    await CartService.add_to_cart(db=session, tenant_id=test_tenant.id, user_id=verified_user.id, product_id=p2.id, quantity=3)
