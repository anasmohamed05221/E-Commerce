...
async def test_update_profile_first_name(session, verified_user):
    ...
    updated = await UserService.update_profile(session, verified_user, data)

    assert updated.first_name == "Updated"
    assert updated.last_name == original_last_name


async def test_update_profile_multiple_fields(session, verified_user):
...
async def test_update_profile_unset_fields_unchanged(session, verified_user):
